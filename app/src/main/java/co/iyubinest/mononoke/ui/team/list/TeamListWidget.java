...
class TeamListWidget extends RecyclerView {
  ...
  public TeamListWidget(Context context, @Nullable AttributeSet attrs) {
    ...
    TypedArray typedArray = context.obtainStyledAttributes(
      attrs,
      R.styleable.TeamListWidget,
      0,
      0
    );
    int spanCount = typedArray.getInt(
      R.styleable.TeamListWidget_columns,
      1
    );
  ...
  private static class TeamListAdapter extends RecyclerView.Adapter<TeamListHolder> {
    ...
    @Override
    public TeamListHolder onCreateViewHolder(final ViewGroup parent, int viewType) {
      return new TeamListHolder(LayoutInflater.from(parent.getContext()).inflate(
  ...
  static class TeamListHolder extends RecyclerView.ViewHolder {
    @BindView(R.id.team_list_item_name)
    TextView nameView;
    @BindView(R.id.team_list_item_avatar)
    ImageView avatarView;
    @BindView(R.id.team_list_item_github)
    TextView githubView;
    @BindView(R.id.team_list_item_location)
    ImageView locationView;
    @BindView(R.id.team_list_item_role)
    TextView roleView;
    @BindView(R.id.team_list_item_languages)
    TextView languagesView;
    @BindView(R.id.team_list_item_tags)
    TextView tagsView;
    @BindView(R.id.team_list_item_status)
    TextView statusView;
