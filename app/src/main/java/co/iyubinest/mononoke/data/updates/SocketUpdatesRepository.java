...
public class SocketUpdatesRepository implements UpdatesRepository {
  ...
  public SocketUpdatesRepository(Moshi moshi,
                                 RxSocket socket,
  ...
  public Flowable<TeamEvent> get() {
    return socket.get().flatMap(msg -> users.flatMap(users -> roles.get().flatMap(roles -> toEvent(
      msg,
      users,
      roles
    ))));
  ...
  private Flowable<TeamEvent> newUserResponseWith(Object response, Map<String, String> roles) {
    ...
    final User newUser = BasicUser.create(
      newUserResponse.name,
      newUserResponse.avatar,
      newUserResponse.github,
      roles.get(String.valueOf(newUserResponse.role)),
      newUserResponse.location,
      "",
      newUserResponse.languages,
